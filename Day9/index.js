...
var calcScreendiv1 = createNewDiv("calc-operation", " ");
...
var buttonCalc = createNewDiv("button l eval", "=", "performcalc()");
...
document.body.addEventListener("keydown", (e) => {
  let char = e.key;
  if (
    possible.includes(String(char)) &&
    char !== "Backspace" &&
    char !== "=" &&
    char !== "c"
  ) {
    document.getElementById("operations").innerText += char;
    // expression = test.value;
  }
  if (
    possible.includes(char) &&
    char === "Backspace" &&
    document.getElementById("operations").innerText.length !== 0
  ) {
    clearOneCharOperStr();
  }
  if (possible.includes(char) && char === "=") {
    performcalc();
  }
  if (possible.includes(char) && char === "c") {
    clearOperStr();
  } else {
    console.log("No");
  }
